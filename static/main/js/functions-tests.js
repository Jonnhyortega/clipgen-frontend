let progress = 0;
let currentStep = 0;
const currentStepContainer = document.getElementById("current-step-info");
const progressMapping = [
  ...
  "Creando video",
  "Guardando video en la base de datos",
  "Subiendo video a S3",
...
function updateProgress() {
  ...
  currentStepContainer.innerHTML = progressMapping[currentStep];
  if (progress <= 100) {
    progress += 1;
    ...
    const newStep = Math.floor(progress / (100 / progressMapping.length));

    if (newStep > currentStep && newStep < progressMapping.length) {
      currentStep = newStep;
      currentStepContainer.innerHTML = progressMapping[currentStep];
    }
  }
...
// INICIALIZACION DE FUNCIONES PARA RENDERIZAR UNA
// SIMULACION DEL CONTENIDO DEL SERVIDOR AL CUAL NO SE
//  TUVO ACCESO PARA DESARROLLAR EL FRONT END
